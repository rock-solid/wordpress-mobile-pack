...
function WMP_SEND_FEEDBACK() {
  ...
  this.validate = function() {
    ...
    if (
      !jQuery('#' + JSObject.type + '_permissions_email', JSObject.DOMDoc).is(
        ':checked'
      ) &&
      !jQuery(
        '#' + JSObject.type + '_permissions_directemail',
        JSObject.DOMDoc
      ).is(':checked')
    ) {
      alert('Please confirm that we can contact you over email');
  ...
  this.submitForm = function() {
    return WMPJSInterface.AjaxUpload.dosubmit(JSObject.form, {
  ...
  this.completeUploadingData = function(response) {
    ...
    if (response) {
      ...
      // show message
      var message =
        'We were unable to send the message because your mail() function is probably disabled. Please send your message directly to <a href="mailto:' +
        JSObject.feedbackEmail +
        '">' +
        JSObject.feedbackEmail +
        '</a>.';
