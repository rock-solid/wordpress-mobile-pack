    ...
    class PtPwa
    {
		 ...
		 */
		 public function display_icon_reupload_notice(){
			...
			if ($icon_filename == '') {
				echo '<div class="notice notice-warning is-dismissible"><p>Publishers Toolbox PWA: Upload an <a href="' . get_admin_url() . 'admin.php?page=wmp-options-theme-settings"></a>App Icon</a> to take advantage of the Add To Home Screen functionality!</p></div>';
				...
				foreach (PtPwa_Uploads::$manifest_sizes as $manifest_size) {
					if (!file_exists(PWA_FILES_UPLOADS_DIR . $manifest_size . $icon_filename)) {
						echo '<div class="notice notice-warning is-dismissible"><p>Publishers Toolbox PWA comes with Add To Home Screen functionality which requires you to reupload your <a href="' . get_admin_url() . 'admin.php?page=wmp-options-theme-settings"></a>App Icon</a>!</p></div>';
						return;
					}
				}
			}
