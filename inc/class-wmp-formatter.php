...
if ( ! class_exists( 'WMobilePack_Formatter' ) ) {
    ...
    class WMobilePack_Formatter
    {
        ...
        public static function init_purifier()
        {
            ...
            $config->set('URI.SafeIframeRegexp', "%^(https?:)?(http?:)?//(www\.youtube(?:-nocookie)?\.com/embed/|player.vimeo.com|www\.dailymotion.com|w.soundcloud.com|fast.wistia.net|fast.wistia.com|wi.st|flickrit.com)%");

            // extend purifier
            $Html5Purifier = new WMPHtmlPurifier();
            return $Html5Purifier->wmp_extended_purifier($config);
        }
