...
function EDIT_CATEGORIES(){
	...
	this.initServerValidate = function(){
		...
		var $Email = jQuery('#'+this.type+'_email',this.DOMDoc);
		$Email.trigger("blur");
		
		//wait until the email is verified with AJAX
		var interval = setInterval(function(){
			
			if ($Email.data("ajaxResponse") == true){

				clearInterval(interval)
				
				//if all responses from the server are true for all requested fields, then do submit
				if ( JSObject.validator.errorList.length == 0 ){
					JSObject.sendData();
				}
				else{
					arr_errorsYCoord.push($Email.offset().top);
	...
	this.completeUploadingData = function(responseJSON){
		...
		response = Number(String(JSON.status));
